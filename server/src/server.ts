...
// Function to get all users in a room
function getUsersInRoom(roomId: string): User[] {
  console.log(`user joined the room ${roomId}`); // log 1
  return userSocketMap.filter((user) => user.roomId == roomId);
}
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnecting", () => {
    ...
    if (!user) return;
    const roomId = user.roomId;
    socket.broadcast.to(roomId).emit(MessageEvent.USER_DISCONNECTED, { user });
    userSocketMap = userSocketMap.filter((u) => u.socketId !== socket.id);
    console.log(`user left the room ${roomId}`); // log2
    socket.leave(roomId);
  });
  ...
  socket.on(MessageEvent.FILE_CREATED, ({ file }) => {
    const roomId = getRoomId(socket.id);
    if (!roomId) return;
    console.log(`file was created by ${socket.id} in room ${roomId}`); // log 3
    socket.broadcast.to(roomId).emit(MessageEvent.FILE_CREATED, { file });
  });
  ...
  socket.on(MessageEvent.FILE_UPDATED, ({ file }) => {
    const roomId = getRoomId(socket.id);
    if (!roomId) return;
    console.log(`file was updated by ${socket.id} in room ${roomId}`); // log 4
    socket.broadcast.to(roomId).emit(MessageEvent.FILE_UPDATED, { file });
  });
  ...
  socket.on(MessageEvent.FILE_RENAMED, ({ file }) => {
    const roomId = getRoomId(socket.id);
    if (!roomId) return;
    console.log(`file was renamed by ${socket.id} in room ${roomId}`); // log 5
    socket.broadcast.to(roomId).emit(MessageEvent.FILE_RENAMED, { file });
  });
  ...
  socket.on(MessageEvent.FILE_DELETED, ({ id }) => {
    const roomId = getRoomId(socket.id);
    if (!roomId) return;
    console.log(`file was deleted by ${socket.id} in room ${roomId}`); // log 6
    socket.broadcast.to(roomId).emit(MessageEvent.FILE_DELETED, { id });
  });
  ...
  socket.on(MessageEvent.USER_OFFLINE, ({ socketId }) => {
    ...
    const roomId = getRoomId(socketId);
    if (!roomId) return;
    console.log(`message was send by ${socket.id} in room ${roomId}`); // log 7
    socket.broadcast.to(roomId).emit(MessageEvent.USER_OFFLINE, { socketId });
  });
  ...
  socket.on(MessageEvent.SEND_MESSAGE, ({ message }) => {
    ...
    if (!roomId) return;
    console.log(
      `message was send by ${socket.id} in room ${roomId} : ${message}` // log 8
    );
    socket.broadcast.to(roomId).emit(MessageEvent.RECEIVE_MESSAGE, { message });
  });
  ...
  socket.on(MessageEvent.TYPING_START, ({ cursorPosition }) => {
    ...
    if (!user) return;
    const roomId = user.roomId;
    console.log(`message position : ${cursorPosition}`); // log 9
    socket.broadcast.to(roomId).emit(MessageEvent.TYPING_START, { user });
  });
